import { ref, reactive, provide, computed, watch, onMounted, onActivated } from 'vue';
...
var useRolesStore = defineStore('roles', function () {
    var roles = ref([]);
...
function CRUD(crudOptions, notification) {
     ...
     *  @description 钩子函数
     */
    var crudHook = reactive(Object.assign({}, HOOK));
    /**
     *  @description 数据主键，默认从接口返回数据中取该值存入 crudResData 中
     */
    var crudPrimarykey = ref('id');
     ...
     *  @description form表单DOM标识<crudDefaultForm>
     */
    var crudDefaultFormRef = ref();
    /**
     *  @description 表单数据 （从表格获取，提交时使用）
     */
    var crudForm = reactive({});
    /**
     *  @description 初始化表单数据 （新增时初始化代理表单数据）
     */
    var defaultForm = reactive({});
     ...
     */
    var proxyForm = reactive({});
    /**
     *  @description 查询条件
     */
    var crudQuery = reactive({});
    /**
     *  @description 初始化查询条件数据
     */
    var defaultQuery = reactive({});
    ...
    provide('crudStatus', computed(function () { return crudStatus; }));
    /**
    ...
    // 导出的 Loading
    var downloadLoading = ref(false);
    // 删除的 Loading
    var delLoading = ref(false);
    // 多选删除的 Loading
    var delAllLoading = ref(false);
    var crudPage = reactive({
        page: 1,
        limit: 10,
        count: 0
    });
    ...
    var addEditLoading = ref(false);
    watch(function () { return crudStatus.cu; }, function () {
